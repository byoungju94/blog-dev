...
public interface PostRepository extends PagingAndSortingRepository<Post, Long>, PostReadRepository {
    ...
    List<Post> findByCategoryIdWithPaging(@Param("categoryId") String categoryId, Pageable pageable);

    @Query("SELECT p.* FROM post p WHERE p.categoryId = :categoryId AND p.title LIKE '%:title%' AND p.id > 0")
    List<Post> findByCategoryIdAndTitleWithPaging(@Param("categoryId") String categoryId, @Param("title") String title, Pageable pageable);
}
